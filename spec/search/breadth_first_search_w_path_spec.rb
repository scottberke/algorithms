...
describe 'Breadth First Search With Path' do
  let(:tree) do
    #            50
    #        /        \
    #       15         70
  ...
  context 'when node is in tree' do
    it 'finds the correct node and the path' do
      node_container = breadth_first_search_w_path(tree, 18)
      expect(node_container.node.data).to eq 18
      expect(node_container.path).to eq [50, 15, 18]
    end
  end

  context 'when node is NOT in tree' do
    it 'returns nil' do
      node = breadth_first_search_w_path(tree, 19)
